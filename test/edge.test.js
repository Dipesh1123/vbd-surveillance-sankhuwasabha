...
/* --- no sign-in: every handler is open ----------------------------------- */
run(`
...
run(`apiSavePulse({ palika:'Khandbari Municipality', report_date:YDAY,
  dengue_ns1:3, dengue_positives:0 })`);
const moved = run(`apiSaveCase({ case_id:CASE1, palika:'Khandbari Municipality',
...
const yday = run("apiGetPulse('Khandbari Municipality',YDAY)");
check("yesterday was not corrupted", yday.data.issues.dengue.entered === 0, yday.data.issues.dengue);
const still = run("apiGetPulse('Khandbari Municipality',TODAY)");
...
const moved2 = run(`apiSaveCase({ case_id:CASE1, palika:'Khandbari Municipality',
  disease:'dengue', patient_name:'Case One', age:30, age_unit:'years', sex:'Male',
  ward:1, test_type:'NS1', test_date:YDAY })`);
check("edit onto a backed day is allowed", moved2.ok && moved2.data.saved === true, moved2);
check("yesterday now has the case",
  run("apiGetPulse('Khandbari Municipality',YDAY)").data.issues.dengue.entered === 1);
check("today now short by one",
  run("apiGetPulse('Khandbari Municipality',TODAY)").data.issues.dengue.level === "warn");

console.log("\n--- 4. Changing disease without room is refused ---");
const swap = run(`apiSaveCase({ case_id:CASE1, palika:'Khandbari Municipality',
...
const inPlace = run(`apiSaveCase({ case_id:CASE1, palika:'Khandbari Municipality',
...
console.log("\n--- 6. A cross-palika move is allowed, but still needs quota ---");
/* Moving a case to another palika is no longer a permission question. It is
   still a reconciliation question: the destination day must have declared room
   for it, or the line list would exceed what the daily return reported. That
   rule is the one thing standing between an open API and meaningless
   positivity figures, so it matters more here than it did before. */
const cross = run(`apiSaveCase({ case_id:CASE1, palika:'Madi Municipality',
  disease:'dengue', patient_name:'Case One Renamed', age:33, age_unit:'years',
  sex:'Male', ward:7, test_type:'IgM', test_date:YDAY })`);
check("the move is refused for want of a declared positive, not for permission",
  cross.ok === true && cross.data.saved === false && !/palika/i.test(cross.data.message || ""),
  cross.ok ? cross.data.message : cross);
check("the case stayed where it was",
  run("findByKey('Cases','case_id',CASE1).palika") === "Khandbari Municipality",
  run("findByKey('Cases','case_id',CASE1).palika"));
...
const xss = run(`apiSaveCase({ palika:'Khandbari Municipality', disease:'dengue',
...
const csvOut = run("apiExport('linelist',{})").data.csv;
...
const uni = run(`apiSaveCase({ palika:'Khandbari Municipality', disease:'scrub',
...
const uni2 = run(`apiSaveCase({ palika:'Khandbari Municipality', disease:'scrub',
  patient_name:'सविना राई', age:22, age_unit:'years', sex:'Female', ward:3,
  tole:'तुम्लिङटार', test_type:'IgM RDT', test_date:TODAY })`);
...
console.log("\n--- 8. Age validation ---");
const oldAge = run(`apiSaveCase({ palika:'Khandbari Municipality', disease:'dengue',
...
const badMonths = run(`apiSaveCase({ palika:'Khandbari Municipality', disease:'dengue',
...
const zeroAge = run(`apiSaveCase({ palika:'Khandbari Municipality', disease:'dengue',
...
const unlimited = run(`apiSavePulse({ palika:'Khandbari Municipality',
...
console.log("\n--- 12. Pagination ---");
run(`
  apiSavePulse({ palika:'Madi Municipality', report_date:TODAY,
    dengue_ns1:80, dengue_positives:60 });
  for (var i = 0; i < 60; i++) {
    apiSaveCase({ palika:'Madi Municipality', disease:'dengue',
      patient_name:'Bulk ' + i, age:20, age_unit:'years', sex:'Male', ward:1,
      test_type:'NS1', test_date:TODAY });
  }
`);
const p0 = run("apiListCases({ scope:'Madi Municipality', page:0, perPage:50 })");
...
const p1 = run("apiListCases({ scope:'Madi Municipality', page:1, perPage:50 })");
...
check("perPage is capped at 200",
  run("apiListCases({ scope:'Madi Municipality', page:0, perPage:9999 }).data.perPage") === 200);

console.log("\n--- 13. The line list spans every palika ---");
/* perPage 200 rather than the default 50: the bulk load above put 60 Madi
   cases at the top of the sort, so page one alone proves nothing about scope. */
const mine = run("apiListCases({ perPage:200 })");
check("an unscoped list spans every palika that has cases",
  new Set(mine.data.rows.map(r => r.palika)).size > 1,
  [...new Set(mine.data.rows.map(r => r.palika))]);
check("scope still narrows to one palika when asked",
  run("apiListCases({ scope:'Madi Municipality' })").data.rows
    .every(r => r.palika === "Madi Municipality"));
check("search matches any palika's patient by name",
  run("apiListCases({ query:'Bulk' })").data.total > 0,
  run("apiListCases({ query:'Bulk' })").data.total);
check("names are returned in full, never masked",
  mine.data.rows.every(r => r.patient_name && r.patient_name.indexOf("•") === -1),
  mine.data.rows.map(r => r.patient_name).slice(0, 3).join(" | "));
...
run(`apiSavePulse({ palika:'Madi Municipality', report_date:TODAY,
  dengue_ns1:80, dengue_positives:62 })`);
const before = run("readAll('Cases').length");
const doomed = run(`apiSaveCase({ palika:'Madi Municipality', disease:'dengue',
  patient_name:'To Delete', age:44, age_unit:'years', sex:'Male', ward:1,
  test_type:'NS1', test_date:TODAY })`);
run(`apiDeleteCase(${JSON.stringify(doomed.data.case_id)})`);
const next = run(`apiSaveCase({ palika:'Madi Municipality', disease:'dengue',
...
const busy = run(`apiSavePulse({ palika:'Khandbari Municipality', report_date:TODAY,
...
const okAfter = run(`apiSavePulse({ palika:'Khandbari Municipality', report_date:TODAY,
  dengue_suspects:5, dengue_ns1:4, dengue_positives:2, scrub_rdt:2, scrub_positives:1 })`);
...
check("it no longer issues any codes", boot && boot.codes === undefined, boot && boot.codes);
check("it no longer sets a district code", boot && boot.districtCode === undefined, boot && boot.districtCode);
check("it still reports the web app URL", boot && typeof boot.url === "string", boot && boot.url);
check("no code column survives in the sheet",
  run("readAll('Units').every(function(u){ return u.code_hash === undefined && u.code_salt === undefined; })"),
  run("Object.keys(readAll('Units')[0] || {}).join(',')"));
check("the code generator is gone too", run("typeof newAccessCode_") === "undefined");
